...
/** Add or change full-team group photos per year (admin only). */
export function TeamPhotoForm({
  ...
  const [photo, setPhoto] = useState(team.years[0]?.photo ?? "");
  ...
  useEffect(() => {
    ...
    setYearLabel(y.year);
    setPhoto(y.photo ?? "");
    // Only re-sync when team data refreshes from parent (after save)
  ...
  const savePhoto = async () => {
    ...
    try {
      const years = team.years.map((y) =>
        y.year === yearLabel ? { ...y, photo: photo || undefined } : y,
      );
      // If year somehow missing from list, don't invent a full roster
  ...
  const addYear = async () => {
    ...
    try {
      ...
      // Newest year first (carousel index 0)
  ...
  const clearPhoto = async () => {
    if (!yearLabel) return;
    if (!confirm(`Remove the full team photo for ${yearLabel}?`)) return;
    setPhoto("");
    setStatus("saving");
    setErrorMsg(null);
    try {
      const years = team.years.map((y) =>
        y.year === yearLabel ? { ...y, photo: undefined } : y,
      );
      await persist(years);
      setStatus("saved");
      setTimeout(() => setStatus("idle"), 2500);
  ...
  return (
    <div className="max-w-2xl space-y-4 rounded-2xl bg-white p-6 shadow-card-sm">
      <h2 className="text-base font-bold text-ink">Full team photo</h2>
      <p className="text-xs text-ink/55">
        Group shot shown in the Team page carousel. Roster for each year is unchanged when you
        update the photo.
      </p>
        ...
        >
          {team.years.map((y) => (
            <option key={y.year} value={y.year}>
              {y.year}
              {y.photo ? "" : " (no photo yet)"}
            </option>
          ))}
        </select>
      </label>

      <ImageField label="Group photo" kind="team" value={photo} onChange={setPhoto} />
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {photo && (
          <button
            ...
            className="rounded-lg bg-red-50 px-4 py-2.5 text-sm font-bold text-red-600 hover:bg-red-100 disabled:opacity-50"
          >
            Remove photo
          </button>
        )}
      ...
      <div className="border-t border-[#eee4d6] pt-4">
        <p className="text-xs font-semibold text-ink">Add a year (empty roster + photo slot)</p>
